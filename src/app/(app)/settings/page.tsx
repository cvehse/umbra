...
type Src = { name: string; role: string; state: "live" | "keyed" | "paid" | "keyless-optional" };
...
function sources(): Src[] {
  ...
  return [
    ...
    { name: "NVD", role: "CVE / vulnerability feed", state: env.NVD_API_KEY ? "keyed" : "live" },
    { name: "GitHub", role: "Public code exposure" + (env.GITHUB_TOKEN ? " + secret scanning" : ""), state: env.GITHUB_TOKEN ? "keyed" : "keyless-optional" },
    { name: "abuse.ch ThreatFox", role: "Malware IOC feed", state: env.THREATFOX_API_KEY ? "keyed" : "keyless-optional" },
    { name: "DeHashed", role: "Per-record credentials & PII (paid)", state: env.DEHASHED_API_KEY ? "keyed" : "paid" },
    { name: "Intelligence X", role: "Deep dark-web / forum / paste (paid)", state: env.INTELX_API_KEY ? "keyed" : "paid" },
  ];
}

const stateMeta: Record<Src["state"], { label: string; cls: string }> = {
    ...
    <div className="space-y-6">
      ...
      <div className="panel p-5">
        ...
        <p className="mb-4 text-xs text-ink-faint">
          ...
          <span className="mono text-ink-soft">.env.local</span> to unlock higher limits, secret scanning and paid per-record feeds.
        </p>
