...
const maskPw = (p: string) => (!p ? "—" : p.length <= 2 ? "••" : p.slice(0, 1) + "•".repeat(Math.min(10, p.length - 1)));

export function ScanView({ initialDomain = "" }: { initialDomain?: string }) {
  const [domain, setDomain] = useState(initialDomain);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<DomainScan | null>(null);
...
function Results({ result, onRescan }: { result: DomainScan; onRescan: () => void }) {
  ...
  return (
    <div className="space-y-5 animate-fadeup">
      ...
      {/* fresh stealer exposure (Hudson Rock) */}
      ...
      {/* leaked records — the actual data (historical) */}
...
function StealerSection({ intel }: { intel: import("@/lib/live/scan").StealerIntel }) {
...
function Inline({ icon: Icon, value, label, tone }: { icon: typeof Users; value: string; label: string; tone: string }) {
  return (
    <div>
...
function RecordsSection({ result }: { result: DomainScan }) {
...
function SubdomainSection({ subs }: { subs: string[] }) {
...
function Section({ title, count, icon: Icon, tone, actions, children }: {
  title: string; count?: number; icon: typeof Database; tone: string; actions?: React.ReactNode; children: React.ReactNode;
}) {
  return (
    <section>
...
function ScanSkeleton({ domain }: { domain: string }) {
  return (
