...
import type { DomainScan } from "@/lib/live/scan";

const KEY = "umbra.watchlist";
type Row = { domain: string; loading: boolean; scan?: DomainScan; error?: string };
...
export function Watchlist() {
  ...
  const scanOne = useCallback(async (domain: string) => {
    ...
    try {
      ...
      setRows((prev) => prev.map((r) => (r.domain === domain ? { ...r, loading: false, scan: data } : r)));
    } catch (e) {
      setRows((prev) => prev.map((r) => (r.domain === domain ? { ...r, loading: false, error: e instanceof Error ? e.message : "failed" } : r)));
    }
  }, []);
  ...
  useEffect(() => {
    let domains: string[] = [];
    try { domains = JSON.parse(localStorage.getItem(KEY) || "[]"); } catch { domains = []; }
    if (domains.length === 0) domains = ["adobe.com", "dropbox.com"]; // sensible starter
    setRows(domains.map((d) => ({ domain: d, loading: true })));
    setReady(true);
    domains.forEach((d) => scanOne(d));
  }, [scanOne]);

  const persist = (domains: string[]) => { try { localStorage.setItem(KEY, JSON.stringify(domains)); } catch { /* ignore */ } };

  const add = () => {
    const d = input.trim().toLowerCase().replace(/^https?:\/\//, "").replace(/^www\./, "").replace(/\/.*$/, "");
    if (!d || rows.some((r) => r.domain === d)) { setInput(""); return; }
    const next = [{ domain: d, loading: true }, ...rows];
    setRows(next); persist(next.map((r) => r.domain)); setInput("");
    scanOne(d);
  };
  const remove = (domain: string) => {
    const next = rows.filter((r) => r.domain !== domain);
    setRows(next); persist(next.map((r) => r.domain));
  };
  ...
  return (
    <div className="space-y-4">
          ...
          <div key={r.domain} className="panel panel-hover p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-4">
                ...
                <button onClick={() => scanOne(r.domain)} className="text-ink-faint hover:text-ink" title="Re-scan"><RefreshCw size={15} /></button>
                <Link href={`/scan?domain=${encodeURIComponent(r.domain)}`} className="text-ink-faint hover:text-brand-soft" title="Full report"><ArrowRight size={16} /></Link>
      ...
      <p className="text-[11px] text-ink-faint">Watchlist is stored in your browser. In production this is per-organization with scheduled re-scans and alerting when new exposure appears.</p>
    </div>
  );
}

function Metric({ icon: Icon, value, tone, title }: { icon: typeof KeyRound; value: number; tone: string; title: string }) {
  return (
